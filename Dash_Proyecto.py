...
#Mapa
figMapCant_swl_5 = px.choropleth(dfcant_swl_5, locations="sector",
                            locationmode="country names",
                            color="amount", 
                            hover_name="sector", 
                            color_continuous_scale=["#99ccff", "#ff3333"])   
...
dfprom_ganwls = pd.DataFrame(query_8, columns=["sector", "average_revenue"])

#Grafico pie
figPieProm_ganwls = px.pie(dfprom_ganwls.head(20), names="sector", values="average_revenue")
#Grafico linea
figLineProm_ganwls = px.line(dfprom_ganwls.head(20),  x="sector", y="average_revenue")
#Mapa
figMapProm_ganwls = px.choropleth(dfprom_ganwls, locations="sector",
                            locationmode="country names",
                            color="average_revenue", 
                            hover_name="sector", 
                            color_continuous_scale=["#99ccff", "#ff3333"])  
...
dfprom_prec = pd.DataFrame(query_9, columns=["sector", "average_price"])

#Grafico pie
figPieProm_PWLS = px.pie(dfprom_prec.head(20), names="sector", values="average_price")
#Grafico linea
figLineProm_PWLS = px.line(dfprom_prec.head(20),  x="sector", y="average_price")
#Mapa
figMapProm_PWLS = px.choropleth(dfprom_prec, locations="sector",
                            locationmode="country names",
                            color="average_price", 
                            hover_name="country", 
                            color_continuous_scale=["#99ccff", "#ff3333"])  
            ...
            html.Div(className= "col-12 col-xl-6", style={'marginBottom': '20px'}, children=[
                html.Div(className= "card border-info border-dark",  style={'backgroundColor':'#292C6D', 'color':'#fff'}, children=[
                    html.Div(className= "card-header", style={'backgroundColor':'#EC255A'}, children=[
                            html.H2(children='Cantidad Empresas (SWL)', style={'textAlign': 'center'}),    
                    ]),
                    html.Div(className= "card-body", children=[
                                dcc.Graph(
                                    id='barCant_swl_5',
                                    figure=figBarCant_wls1
                                ),    
                    ]),    
